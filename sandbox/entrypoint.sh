...
if [ "$GPG_SIGNING" = "true" ] && [ -n "$GIT_AUTHOR_NAME" ] && [ -n "$GIT_AUTHOR_EMAIL" ]; then
  ...
  if [ -n "$existing_email" ] && [ "$existing_email" != "$GIT_AUTHOR_EMAIL" ]; then
    ...
    echo "  docker run --rm -v claude-sandbox:/data -v \"\$(pwd):/output\" local/claude-sandbox \\"
    echo "    bash -c 'gpg --homedir /data/.gnupg --export-secret-keys --armor > /output/my-key-backup.asc'"
    echo ""
    echo "This creates my-key-backup.asc in your current directory."
